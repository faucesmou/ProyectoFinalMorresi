...
//firebase:
import { doc, getDocs, getFirestore, collection, query, where, getDoc, addDoc } from "firebase/firestore";


function ItemListContainer({ products }) {

//primera interacción con base de datos: 
...
/* useEffect(()=> {
  ...
  const biciRef = doc(db, "items", "8m97TYB83MVJtg5Tyedt");
  getDoc(biciRef).then((snapshot)=> {
    if (snapshot.exists()){
      setProduct({ id:snapshot.id, ...snapshot.data() });
      console.log('este es el estado de product: ', product)
...
const insertarDatos =() => {
  ...
  for ( let item of data) {
    ...
    .then(res => console.log('se insertó en la base de datos', res.id ))
...
//hasta acá la función de insertar

/* lógica para la base de datos y qué productos mostrar para cada categoría: */
/* 
